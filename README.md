...
HyperLight — это модуль Xposed, который завершает эффекты размытия подсветки для HyperOS 3, обеспечивая единый визуальный опыт для рабочего стола, шторки уведомлений и центра управления.

### Возможности

**Системный лаунчер (com.miui.home)**
- Эффект размытия подсветки папок

**Системный интерфейс (com.android.systemui)**
- Эффект размытия подсветки шторки уведомлений

**Центр управления (miui.systemui.plugin)**
- Эффект размытия подсветки центра управления
- Поддержка эффекта тени
- Настройка цвета иконок (Светлый/Тёмный/Как в системе)

### Совместимость

- Целевая версия Android: Android 16
- Поддерживаемая система: HyperOS 3.x
- Фреймворк Xposed: LSPosed / Xposed

### Установка

1. Установите фреймворк Xposed (рекомендуется LSPosed)
2. Скачайте и установите модуль HyperLight
3. Активируйте модуль в Xposed Manager
4. Выберите области применения (scopes): `com.miui.home` и `com.android.systemui`
5. Перезапустите Системный лаунчер и Системный интерфейс
6. Откройте приложение HyperLight для настройки

### Сообщения об ошибках

Пожалуйста, оставляйте обратную связь в Issues, указав следующую информацию:

1. Логи фреймворка Xposed (LSPosed: Настройки → Логи → Подробные логи)
...
Примечание: Проблемы без предоставления логов могут быть не эффективно решены.
...
- MiuiX — библиотека UI-компонентов

### Разработчик

愛君の名は

### Перевод на русский язык: Majestic`s Translation(https://t.me/majestics_translation)
